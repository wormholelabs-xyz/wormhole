...
Develop a mechanism for a set of keys designated by the guardians can be granted the authority to pause and unpause Wormhole protocols, such as WTT (Token Bridge) and NTT.
...
- Pauser role for WTT which can prevent all contract actions except for unpausing
- Unpauser role for WTT
...
- Update WTT (Token Bridge) to include a Pauser and Unpauser role and respective `pause` / `unpause` method and functionality
...
1. Verify a Pauser governance message and update the signers, threshold, expiryDuration, and config index
   1. MUST Validate that the action matches this platform
   ...
   7. Signers are defined by the built-in sender verification mechanism on the platform, e.g. `msg.sender` for EVM, `acct.is_signer` on SVM, etc.
      ...
      2. MUST validate that no signer is the zero address / all-zero pubkey
      3. MUST validate that the signer list contains no duplicates
...
1. Add a `notPaused` check to every entry point that is not governance or `unpause`
2. Add new governance message handling and corresponding state to set the `pauser` and `unpauser` role
   1. MUST validate the action matches this platform and the chain ID matches this chain
   2. MUST check and mark replay protection for the governance message, consistent with the rest of the token bridge governance handling
3. Add a new `pause` entry point that can only be called by the `pauser` and sets the `paused` state to `true`
4. Add a new `unpause` entry point that can only be called by the `unpauser` and sets the `paused` state to `false`
...
//   action:   uint8 - 1 = SetConfigEvm, 2 = SetConfigSolana
//   chainId:  uint16 target chain

// Action 1: EVM signers (20-byte addresses)
// Wire layout (after header):
//   index:          uint16  (must equal on-chain index + 1; first valid is 1)
//   threshold:      uint8
//   expiryDuration: uint64
//   numSigners:     uint8
//   signers:        bytes20[numSigners]
struct BodyDelegatedPauserSetConfigEvm {
    uint16 index;
    uint8 threshold;
...
// Action 2: Solana signers (32-byte pubkeys)
// Wire layout (after header):
//   index:          uint16  (must equal on-chain index + 1; first valid is 1)
//   threshold:      uint8
//   expiryDuration: uint64
//   numSigners:     uint8
//   signers:        bytes32[numSigners]
struct BodyDelegatedPauserSetConfigSolana {
    uint16 index;
    uint8 threshold;
    uint64 expiryDuration;
    bytes32[] signers;
...
```solidity
// Wire header:
//   module:   "TokenBridge" left-padded to 32 bytes
//   action:   uint8 - 4 = SetPauserAddressesEvm, 5 = SetPauserAddressesSolana
//   chainId:  uint16 target chain (the chain whose token bridge is being configured)

// Action 4: EVM token bridge pauser config (20-byte addresses)
struct BodyTokenBridgeSetPauserAddressesEvm {
    bytes20 pauser;
    bytes20 unpauser;
}

// Action 5: Solana token bridge pauser config (32-byte pubkeys)
struct BodyTokenBridgeSetPauserAddressesSolana {
    bytes32 pauser;
    bytes32 unpauser;
}
```
...
This change should be fully backwards compatible on EVM and SVM for both WTT and NTT. For WTT, Solana already passes the `config` on all relevant calls and NTT already incorporates a `pauser` role.
...
Rather than a single boolean `paused` state, WTT could expose finer-grained pauses (e.g. inbound vs. outbound) so that, for example, redemptions can continue while new deposits are blocked. We are not taking this approach now in favor of a simple boolean; granular pause can be added later without breaking the pause/unpause governance defined here.

### Temporary Pause

Instead of a boolean, the `paused` could be set to a timestamp after which the contract is no longer considered paused. This would introduce a slight additional implementation complexity as well as a risk that the contract could become pre-maturely unpaused.
