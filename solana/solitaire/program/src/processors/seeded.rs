...
/// The ownership story:
/// We require every account to have an owner specified for two reasons:
...
/// However, some accounts are owned by other programs. When performing security
/// checks, the ownership is expressed a constraint on the account, and this
/// constraint may allow multiple different owners.
...
/// might check that it's owned by _either_ the SPL token program, or the
/// Token2022 program. However, when creating a token account, we need to
/// know exactly which program is the owner.
///
...
/// accounts that can have multiple owners. This way, assuming that only the
...
impl<'a, 'b: 'a, K, T: AccountSize + Seeded<K> + Keyed<'a, 'b> + SingleOwned> Creatable<'a, K> for T {
    fn create(
        &'a self,
        accs: K,
...
impl<'a, 'b: 'a, K, T: AccountSize + Seeded<K> + Keyed<'a, 'b> + MultiOwned> CreatableWithOwner<'a, K> for T {
    fn create_with_owner(
        &'a self,
        owner: &Pubkey,
        accs: K,
