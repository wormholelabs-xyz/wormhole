...
The Tilt devnet runs a local Stellar network (`stellar/quickstart`, Soroban RPC on port 8000)
with the core contract deployed and the guardian watcher pointed at it:

```bash
tilt up -- --stellar          # enabled by default with --ci
```

- The `stellar` resource runs quickstart plus a deploy sidecar (`scripts/devnet_deploy.sh`)
  that funds a fixed deployer through friendbot and deploys the core contract with a fixed salt.
  The contract id is therefore deterministic, `CAICJIQ7WBO4OMT5C7KJDYSUDU42WB2CCRPKBSNMFX4WEW3DXHKGG4DQ`,
  and the guardian is started with `--stellarRPC http://stellar:8000/soroban/rpc --stellarContract <id>`.
  All constants live under chain `61` in `scripts/devnet-consts.json`; the committed deployer
  secret is a devnet-only key with no value outside this local network.
- The initial guardian set comes from the devnet guardian keys (`INIT_SIGNERS_CSV` generated by
  `scripts/guardian-set-init.sh`), so it follows the `--num` Tilt argument.
- With `--ci_tests`, the `stellar-ci-tests` job deploys `contracts/devnet-emitter`, publishes one
  message through the core contract and waits for the guardians' signed VAA
  (`scripts/devnet_smoke_test.sh`).
- Soroban RPC is port-forwarded to `http://localhost:8000/soroban/rpc`; friendbot is at
