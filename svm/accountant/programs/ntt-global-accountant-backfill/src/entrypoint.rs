...
use accountant_backfill_core::instructions::{backfill_balance, backfill_noreplay};
use accountant_backfill_core::{err, BackfillError};

use crate::{instructions, Instruction, BACKFILL_AUTHORITY};
...
pub fn process_instruction(
    ...
    match Instruction::from_u8(*discriminator) {
        // Shared, program-ID-agnostic handlers from the core crate.
        ...
        // NTT-native map handlers.
        Some(Instruction::BackfillRelayerRegistration) => {
            instructions::backfill_relayer_registration::process(
                program_id,
                accounts,
                rest,
                &BACKFILL_AUTHORITY,
        ...
        Some(Instruction::BackfillTransceiverHub) => {
            instructions::backfill_transceiver_hub::process(
                program_id,
                accounts,
                rest,
                &BACKFILL_AUTHORITY,
        ...
        Some(Instruction::BackfillTransceiverPeer) => {
            instructions::backfill_transceiver_peer::process(
                program_id,
                accounts,
                rest,
                &BACKFILL_AUTHORITY,
