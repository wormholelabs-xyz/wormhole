...
on every write. The shell crate decodes `NTT_BACKFILL_AUTHORITY` from its `env!`
...
that tooling and this program. The rules below complete that contract. Each rule
is an item for the off-chain parity check.
...
`register_relayer_chain` overwrites the `ChainRegistration` PDA on any unused
sequence.
...
governance VAA that registered the relayer emitter. The snapshot holds the
emitter only, so the value comes from the VAA archive.
...
**Balance key is the hub token.** An NTT balance PDA keys on the hub
`(chain, address)` in the `token_chain` and `token_address` seed positions. The
snapshot already holds those balances normalized to eight decimals, which is the
form the operational program writes at quorum. The only conversion the tooling
makes is the decimal string to a 32-byte big-endian value.
...
- [`ntt-global-accountant`](../ntt-global-accountant/README.md) — the
  operational program this image seeds, and the instructions that read the state
  written here.
- [`global-accountant-backfill`](../global-accountant-backfill/README.md) — the
  sibling migration image for the WTT Global Accountant. It holds the four
  shared handlers without the two NTT-only arms.
