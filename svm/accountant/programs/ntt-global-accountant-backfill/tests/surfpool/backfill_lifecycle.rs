//! The NTT backfill program's lifecycle against a fresh surfpool, with the pinned
//! `solana_noreplay.so` co-deployed:
//!
//! 1. `BackfillNoReplay`: three entries across two buckets. The bitmap bits flip and one
//!    `ACCDGST\0` record per entry reaches `meta.logMessages` over real RPC.
//! 2. `BackfillBalance`: two `BalanceAccountLayout` PDAs at the operational seeds.
//! 3. `BackfillModifyBalance`: one `ModifyBalanceLayout` record.
//! 4. `BackfillRelayerChainRegistration`: two chains, each writing the `ChainRegistration`
//!    PDA and the `RegisterChain` record.
//! 5. `BackfillTransceiverHub`: a self-referential hub and a spoke that names it.
//! 6. `BackfillTransceiverPeer`: both directions of one hub/spoke pair.
//! 7. The WTT operator key signs `BackfillTransceiverHub`: the transaction fails with
//!    `UnauthorizedCaller` and `getAccountInfo` on the target PDA errors.
//!
//! Every write is checked for owner, length, rent and layout, so the on-chain bytes are the
//! bytes the NTT operational program reads after the cutover upgrade.
...
use global_accountant_definitions::{
    ...
    ModifyBalanceLayout, NoReplayBitmapAccount, RegisterChainLayout, TransceiverHubLayout,
    TransceiverPeerLayout, Uint256, UNPINNED_GUARDIAN_SET_INDEX,
};
...
#[test]
#[ignore = "spawns surfpool subprocess; run via `just e2e-ntt-backfill`"]
fn surfpool_ntt_backfill_lifecycle() {
    let guard = start_surfpool(SurfpoolOptions::offline("ntt-backfill-lifecycle"));
    ...
    // The payer is the compiled-in `NTT_BACKFILL_AUTHORITY`; the WTT operator key is the
    // stranger, so the rejection also proves the two artifacts are keyed apart.
    let authority = ntt_test_authority_keypair();
    let stranger = test_authority_keypair();
    ...
    // 3. BackfillModifyBalance: the record that arms `modify_balance`'s replay guard.
    ...
    // 4. BackfillRelayerChainRegistration: the registration PDA and the governance record.
    let registrations = [
    ...
    send(
        &rpc,
        "backfill_relayer_chain_registration",
        &[Instruction {
            program_id: id,
            accounts,
            data: wire::encode_chain_registration_batch(
                Arm::BackfillRelayerChainRegistration as u8,
                &registrations,
            ),
        }],
        &[&authority],
    ...
    // 5. BackfillTransceiverHub: the hub names itself, the spoke names the hub.
    let hubs = [
    ...
    accounts.extend(
        hubs.iter()
            .map(|entry| AccountMeta::new(pda::derive(&id, &entry.key()).0, false)),
    );
    send(
        &rpc,
    ...
    for entry in &hubs {
        let account = rpc
            .get_account(&pda::derive(&id, &entry.key()).0)
            .expect("TransceiverHub PDA");
        assert_written(
            &account,
            &id,
    ...
    // 6. BackfillTransceiverPeer: both directions of the hub/spoke pair.
    let peers = [
    ...
    // 7. The WTT operator key on the NTT artifact: the compile-time authority gate runs.
    let stranger_hub = wire::transceiver_hub_entry(BSC, [0x99u8; 32], BSC, [0x99u8; 32]);
    let stranger_pda = pda::derive(&id, &stranger_hub.key()).0;
    let mut accounts = write_head(&stranger.pubkey());
    accounts.push(AccountMeta::new(stranger_pda, false));
    send_expect_error(
        &rpc,
        "backfill_transceiver_hub[stranger]",
        &[Instruction {
            program_id: id,
            accounts,
            data: wire::encode_transceiver_hub_batch(
    ...
    assert!(
        ...
        "rejected BackfillTransceiverHub wrote its target PDA"
    );
}
