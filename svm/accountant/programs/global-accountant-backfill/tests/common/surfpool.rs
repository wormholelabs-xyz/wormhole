...
//!
//! Lightly trimmed copy of `programs/global-accountant/tests/common/mod.rs`.
//! When more programs join the workspace, lift the shared bits into a
//! dedicated `crates/test-utils/` crate.
...
/// `ACCDGST\0` commit-log payloads, return them in encounter order.
pub fn fetch_accdgst_logs(rpc_url: &str, tx_sig: &str) -> Vec<AccdgstEntry> {
    ...
    for entry in logs {
        ...
        if bytes.len() != ACCOUNTANT_DIGEST_LOG_LEN || bytes[..8] != ACCOUNTANT_DIGEST_LOG_TAG {
            continue;
        }
        ...
        out.push((chain, emitter, sequence, digest, gsi));
    }
    out
}
