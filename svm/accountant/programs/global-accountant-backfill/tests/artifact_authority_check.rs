...
//! and only executing it establishes that; a byte scan of the binary is unreliable.
//!
...
fn built_so_accepts_compiled_in_backfill_authority_and_rejects_others() {
    ...
    assert!(
        ...
        "the built .so rejected its own compiled-in BACKFILL_AUTHORITY ({:?}); the authority \
         gate or this probe is broken",
        BACKFILL_AUTHORITY,
    );
