...
//! prefix to the wire format; `RawIxData` reads every remaining byte as-is. Duplicated from
//! `global-accountant`'s identical file: this is a one-shot migration `.so` that builds and
//! deploys independently, per `accountant-backfill-core`'s `commit_log`/`pda_init` rationale.
