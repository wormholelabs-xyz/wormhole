...
pub const SUBMIT_FIXED_LEN: usize = 32 + 4 + 1 + 65;
...
/// [`Self::from_data`], then call [`Self::populate_routing_from_body`] after the
/// digest cross-check.
#[derive(Clone, Copy)]
...
impl ParsedObservation {
    ...
    /// zeroed for [`Self::populate_routing_from_body`] after the digest cross-check.
