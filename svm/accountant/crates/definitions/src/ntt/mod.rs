...
mod tests {
    ...
    use crate::relayer::parse_delivery_instruction;
    ...
    fn parses_ntt_messages_encodings() {
        ...
        for (mode, locking) in [(Mode::Locking, true), (Mode::Burning, false)] {
            ...
            .to_vec_payload();
            let view = TransceiverInfoPayload::from_payload(&info).expect("info view");
            assert_eq!(view.is_locking(), Ok(locking), "{mode}");
            assert_eq!(view.manager_address, [0x11; 32]);
        }
    ...
    fn mainnet_vectors_parse_to_committed_accounting() {
        ...
        for v in vectors {
            ...
            let transfer = parse_ntt_transfer(ntt_payload)
                .unwrap_or_else(|e| panic!("[{label}] transfer: {e:?}"));
            assert_eq!(
                transfer.amount,
