...
/// Entries per BackfillNoReplay tx. With the compact emitter-grouped wire
/// format, 22 same-emitter / single-bucket entries technically fit, but a
/// chunk that spans a 1024-sequence bucket boundary needs one extra 32-byte
/// account meta. 18 is the safe cap that survives even 3-bucket spans
/// without breaching the 1232-byte raw tx limit.
...
/// Walk the catalogue in order and take the first `target` transfers across a
/// configurable set of distinct emitters. Catalogue is pre-sorted by
/// `(chain, emitter, sequence)`, so taking entries in order yields long
/// consecutive runs from the same emitter — exactly the shape the orchestrator
/// will produce in production, and the shape the compact wire format is
/// optimised for. To still exercise multiple emitters within a single probe
/// run, the function rotates through emitters round-robin: take up to
/// `per_emitter` consecutive sequences from each emitter before advancing.
fn read_in_order_transfers(target: usize, per_emitter: usize) -> Vec<TransferEntry> {
    ...
    for line in reader.lines() {
        ...
        if taken_in_current_emitter >= per_emitter {
            // Skip the rest of this emitter's run; the catalogue is sorted so
            // entries from this emitter come contiguously. Continue scanning;
            // the next emitter transition resets the counter.
            skipped_in_current_emitter += 1;
            let _ = skipped_in_current_emitter; // tracked for debug, not used
            continue;
        }
...
fn surfpool_cost_probe_at_scale() {
    ...
    // from each emitter before rotating to the next. With ~40 emitters in the
    // catalogue and TARGET_TRANSFERS = 100k, this drains 4-5 emitters fully
    // and gives us ~100-150 bucket creations across at least 3 emitters —
    // enough to confirm per-bucket rent is stable across emitters, while
    // still producing the long same-emitter runs the compact wire format
    // optimises for.
    ...
    let noreplay_so = {
        ...
        let ws = manifest_dir
            ...
            .unwrap()
            .to_path_buf();
        ws.join("programs/global-accountant/tests/fixtures/solana_noreplay.so")
    };
                        ...
                        if i as u64 % METADATA_SAMPLE_STRIDE == 0 {
                            if let Some(m) = fetch_meta(&rpc_url, &sig, new_buckets) {
                                ...
                                *total_rent.lock().unwrap() += m.rent_lamports;
                                ...
                                // Couldn't fetch meta but tx succeeded — still
                                // count it in the fee tally with the deterministic
                                // 5,000 L base fee, and the rent we expect from
                                // bucket-creation count.
