...
//! `solana_noreplay.so` is vendored under this program's own `tests/fixtures/`
//! and SHA-256-pinned so the suite is self-contained.
...
/// SHA-256 of the vendored `solana_noreplay.so`. The mollusk harness verifies
/// the fixture against this constant before loading; if you regenerate the
/// `.so`, recompute (`shasum -a 256 tests/fixtures/solana_noreplay.so`) and
...
pub fn noreplay_so_path() -> PathBuf {
    ...
    // CARGO_MANIFEST_DIR = .../programs/global-accountant-backfill
