...
fn close_pending_ix_data(emitter: &[u8; 32], sequence: u64) -> Vec<u8> {
    // Wire: discriminator + 32-byte emitter + 8-byte BE sequence.
...
/// `close_pending` trigger (b) reclaims a pending PDA stranded behind a
/// NoReplay mark set by a sibling digest.
#[test]
fn close_pending_stranded_digest_bucket_after_sibling_committed() {
    ...
    // Simulate the D2 sibling reaching quorum by flipping NoReplay directly.
    let mut accounts = accounts_after_d1.clone();
    if let Some(entry) = accounts
        .iter_mut()
...
/// `close_pending` trigger (a): the current guardian set differs from the
/// pending's recorded index. (The expiration-time sub-case is not host-testable
/// since mollusk's Clock defaults to timestamp 0.)
#[test]
fn close_pending_with_expired_set_succeeds() {
    ...
    // GS account with a different index.
...
/// `close_pending` trigger (b): NoReplay marked.
#[test]
...
/// Rent refund into a u64::MAX recipient trips the checked_add guard
/// (`ArithmeticOverflow`).
#[test]
fn close_pending_with_overflowing_rent_recipient_rejects() {
    ...
    // Saturate the rent recipient and pre-mark noreplay (trigger (b)).
    let mut accounts = accounts_after_first.clone();
    if let Some(entry) = accounts
        .iter_mut()
        .find(|(k, _)| *k == scenario.noreplay_bucket_pubkey)
...
/// DoS regression: a guardian_set account not owned by the Core Bridge must
/// reject with `InvalidPda` before reading any bytes, preventing a forged
/// "expired" verdict from closing a live pending PDA.
#[test]
fn close_pending_rejects_spoofed_guardian_set_owner() {
    ...
    // Spoofed gs: index=99 (mismatches recorded 4), attacker-owned.
    let spoofed_gs = {
...
/// Active set and unmarked NoReplay: close fails with `CannotCleanup`.
#[test]
