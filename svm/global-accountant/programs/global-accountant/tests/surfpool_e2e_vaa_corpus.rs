//! Host-side parser sanity check for the surviving VAA fixtures.
//!
//! Runs in-process (no surfpool) so fixture-format regressions surface during
//! `cargo test` long before any surfpool-driven layer boots. The real CPI is
//! exercised by `surfpool_e2e_mainnet_fork` (seq 2211 happy + negative) and by
//! `surfpool_e2e_submit_vaas` (Token Bridge transfer happy path), so a separate
//! corpus playback layer is no longer pulling its weight.
...
fn corpus_fixtures_parse_and_digest_consistently() {
    // Pin the seq 2211 digest against the value used by
    // surfpool_e2e_mainnet_fork.rs. If the parser regresses, this test fails
    // before any surfpool subprocess ever boots.
    ...
    // The Token Bridge transfer fixture is the live e2e source for
    // surfpool_e2e_submit_vaas — verify it parses cleanly here.
