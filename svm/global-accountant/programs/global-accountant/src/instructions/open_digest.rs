...
// inner helper lives in `super::open_digest_inner` so it is unaffected by
// this gate.
#[cfg(not(feature = "test-only-open-digest"))]
compile_error!(
    "open_digest is a test-only entrypoint; production callers go through \
     `submit_observations`'s commit path. Production builds must not compile \
     this module."
);
...
use crate::err;
use crate::instructions::open_digest_inner;
